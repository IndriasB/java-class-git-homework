...
public class Main {
  private static School generateSchool() {
	...
	school.addStudent(new Student("Indrias", "Beyene"));
	school.addStudent(new Student("Matios", "Hiligorgies"));
	school.addStudent(new Student("Joseph", "Kiflom"));
  ...
  private static void createHandlers() {
	...
	delete("/api/students/:id",(request, response)-> {
      ...
      school.removeStudentById(studentToRemove);
      String json = toJson(studentToRemove);
